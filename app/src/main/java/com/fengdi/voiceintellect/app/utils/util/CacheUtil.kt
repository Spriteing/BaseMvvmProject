package com.fengdi.musicplayer.app.util

import android.text.TextUtils
import com.cyl.musicapi.netease.LoginInfo
import com.fengdi.musicplayer.app.bean.UserInfo
...
object CacheUtil {
    ...
    fun getUser(name: String): UserInfo? {
        val kv = MMKV.mmkvWithID("app")
        val userStr = kv.decodeString(name)
        return if (TextUtils.isEmpty(userStr)) {
            null
        } else {
            Gson().fromJson(userStr, UserInfo::class.java)
    ...
    fun setUser(userResponse: UserInfo?) {
        ...
        if (userResponse == null) {
            ...
            kv.encode(userResponse.platform, Gson().toJson(userResponse))
            setIsLogin(true)
        }
    }
     ...
     * 设置账户信息
     */
    fun saveUser(userResponse: UserInfo?, name: String) {
        val kv = MMKV.mmkvWithID("app")
        if (userResponse == null) {
            kv.encode(name, "")
            setIsLogin(false)
        } else {
            kv.encode(name, Gson().toJson(userResponse))
            setIsLogin(true)
        }
    }
